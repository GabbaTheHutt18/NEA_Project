...
namespace NEA_Project.Commands
{
    public class CheckPairCommand : CommandBase
    {
        ...
        private bool RightPair()
        {
            ...
            {
                ID = 0;
                //Because of the way that the Question banks are saved, this statement means that it 
                //is possible for all users to access the Default Question Bank
            }
            else
            {
                ID = _parent.UserID;
            ...
            found = false;
            //returns the expected answers, but because you don't know which textblock is in the "Answer" column
            //and which is in the "Question", have to create 2 variables to cover both scenarios 
            ...
            List<string> CheckWithTextBlock = _parent.Database.ReadData("QuestionBanks", "Answer", 
                $"Question LIKE '{_vm.TextBlockContains}' AND BankName = '{_parent.CurrentQuestionBank}' AND UserID = {ID}", 1);
            ...
            List<string> CheckWithTextBlock2 = _parent.Database.ReadData("QuestionBanks", "Answer", 
                $"Question LIKE '{_vm.TextBlock2Contains}' AND BankName = '{_parent.CurrentQuestionBank}' AND UserID = {ID}", 1);

            // Because the ReadData method sometimes returns null this IF avoids an exception error as it
            // only compares the textblocks content with the value that should have the answer and is not null. 
